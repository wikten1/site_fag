...
Implementado em `src/components/footer-institutional.html`, com estilos em `assets/css/components/footer.css` e comportamento em `assets/js/components/footer.js`; incluído no HTML gerado da home. Referência visual: `design_system.html`, conceito “Precisão técnica. Presença humana.” O nome institucional segue o briefing e a marca fornecida, pois o nome de fundação de saúde presente no catálogo não corresponde a este projeto.
...
| --- | --- |
| Marca | Logo otimizado `assets/images/brand/logo.webp`; original preservado em `references/images/Logo.png`; retorno para `#hero-title` |
