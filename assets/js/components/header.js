/* Shared navigation, page search and motion preference. */
(() => {
  ...
  const homeLinks = [...menu.querySelectorAll('.nav-link[href^="#"]')];
  function syncActive() {
    if (!homeLinks.length) return;
    const sections = homeLinks.map(link => ({ link, target: document.querySelector(link.getAttribute('href')) })).filter(entry => entry.target);
    const visible = sections.filter(({ target }) => target.getBoundingClientRect().top < innerHeight * .7);
    const active = visible.at(-1) || sections[0];
  ...
  navbar.querySelectorAll('a[href^="#"]').forEach(link => link.addEventListener('click', event => {
    if (event.ctrlKey || event.metaKey || event.shiftKey || event.altKey) return;
    setMenu(false);
    const hash = link.getAttribute('href');
    const target = document.querySelector(hash === '#inicio' ? '#hero-title' : hash);
    target?.focus({ preventScroll: true });
  }));
  addEventListener('hashchange', syncActive);
  function syncScroll() { navbar.classList.toggle('is-scrolled',scrollY>24); syncSize(); syncActive(); }
  setMenu(false);
  syncScroll();
  ...
  addEventListener('resize',()=>{syncSize();syncActive();});
  ...
  form.addEventListener('submit',event=>{
    ...
    const entries=[...document.querySelectorAll('main .news-card, main .story-body, main .editorial-opening, main [data-page-search], main .hero-copy, main .institution-card, main .foundation-copy, main .foundation-principle, main .course-card')].map((content,index)=>{
      const target=content.matches('.foundation-principle')?content:content.matches('.story-body')?document.querySelector('.story-heading h1'):content.querySelector('h1,h2,h3');
